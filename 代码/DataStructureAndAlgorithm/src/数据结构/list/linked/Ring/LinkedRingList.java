 ...
 */
public class LinkedRingList<T> extends AbstractList<T> implements List<T> {
    ...
    public T remove(int index) {
        ...
        if (size == 1) {
            ...
            // 获取要删除的节点以及前驱和后继
            remove = getNodeOfIndex(index);
            Node<T> prev = remove.prev;
            Node<T> next = remove.next;

            // 判断是否删除的是first
            /*if (prev == null) */
            if (remove == first) { //环形链表修改 删除的是第first
                first = next;
            } else {
                prev.next = next;
                //remove.prev = null;
            ...
            // 判断删除的是否是last
            /*if (next == null) */
            if (remove == last) { //环形链表修改 删除的是last
                last = prev;
            } else {
                next.prev = prev;
                last.next = first;// 环形链表修改
                //remove.next = null;
            }
        }
        size--;
