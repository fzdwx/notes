...
比如一些论坛下面的帖子里面的，一些广告，可能会遇到，有人频繁的提交垃圾信息到站点里面。

~~~
pfadd key content 如果返回是1，那么说明没有提交过这个数据。
~~~
...
# 28.周活跃用户数、月活跃用户数、年活跃用户数的统计

pfmerge多个hypeloglog，可以算出来周、月、年的活跃用户
...
如果说要记录一下，在系统里面，执行一些特殊操作，每天执行过某个操作的用户有多少个人，操作日志，审计日志，记录下来每个用户每天做了哪些操作，每个用户每天搞一个set里面放他的一些操作日志



bitmap，位图

一个字节是8位的二进制，int是4字节32位，可以直接在redis里面操作二进制数据

可以把网站里面每一种操作，每天执行过的用户都放在一个位图里面，一个用户对应1位
...
# 30.基于GeoHash的你与商铺距离计算的程序

~~~
geoadd	key lo la member
geodist key from to
~~~
...
# 31.陌生人社交APP里的查找附近的人功能实现

~~~
geoadd
georadiusbymember key xxx radius xxx 
~~~
